...
public class Modulation_2 {

	private int maxDepth;

	private List<Coverage> polCoveragesList; // Some coverages got after the modulation of an input coverage
	//private double coverRatioNeeded;// Which is defined as a parameter by user, all ratios of coverages in 'pepCoveragesList' are >= 'coverRatioNeeded'
	...
	public static double minCoverRatio = 0.8;// To decide which coverages will use light matching again in next loop
	public static double maxCoverRatio = 1.0;// To decide which coverages will use light matching again in next loop

	public Modulation_2(int maxDepth) {
		this.maxDepth = maxDepth;
	}
	
	public List<Coverage> modulate (Coverage cov) {
		
		this.polCoveragesList = new ArrayList<Coverage>();
		this.polCoveragesList.add(cov);
	...
	private boolean recursiveModulation(Coverage cov, HashSet<Match> banned, HashSet<Integer> unmovable, int depth) {
		...
		for (Match match : matchsOnUncoveredAtoms) {
			...
			if (cov.getCoverageRatio() == 1.0) {
				this.polCoveragesList.add(cov.clone());
				return true;
			}
			...
			if (this.recursiveModulation(cov, banned, unmovable, depth-1))
				return true;
				  ...
				  if(cov.getCoverageRatio() >= minCoverRatio && cov.getCoverageRatio() <= maxCoverRatio){
					  this.polCoveragesList.add(cov.clone());
				  }
