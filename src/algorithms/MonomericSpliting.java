...
public class MonomericSpliting {
	...
	public static PeptideCovRatioCorr[] pepCovRatioCorrArray;
	
	public MonomericSpliting(FamilyDB families, ChainsDB chains, int removeDistance, int retryCount, Calculator calculator) {
	...
	public Coverage[] computeCoverages (PolymersDB polDB) {		
		...
		PeptideCovRatioCorr pepCovRatioCorr = new PeptideCovRatioCorr();
		int size = 0;
		for(ArrayList<Coverage> cs: this.polsCoveragesList.values()){
			size += cs.size();
		}
		Coverage[] covs = new Coverage[size];
		pepCovRatioCorrArray = new PeptideCovRatioCorr[size];
		...
		for(ArrayList<Coverage> cs: this.polsCoveragesList.values()){
			...
			for(Coverage c : cs){
				...
				pepCovRatioCorr.setPepName(c.getChemicalObject().getName());
				if(this.calculator instanceof CalculatorTM){
					pepCovRatioCorr.setId(String.valueOf(count));
					pepCovRatioCorr.setCovRatio_TM(String.valueOf(c.getCoverageRatio()));
					pepCovRatioCorr.setCovCorrectness_TM(String.valueOf(c.getCorrectness(this.families)));
